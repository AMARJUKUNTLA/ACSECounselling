...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  const stats = useMemo(() => {
    ...
    return { counsellors, branches, total: students.length };
  }, [students]);
    ...
    <div className="flex flex-col flex-1 min-h-0 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 shrink-0">
         ...
         <div className="bg-emerald-50 p-6 rounded-[2rem] border border-emerald-100">
            <p className="text-[10px] font-black text-emerald-600/60 uppercase tracking-widest">Selected Group</p>
            <p className="text-3xl font-black text-emerald-700 mt-1">{displayedStudents.length}</p>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1 min-h-0">
        ...
        <div className="lg:col-span-4 flex flex-col min-h-0 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
          ...
          <div className="overflow-y-auto custom-scrollbar flex-1">
            <button 
              ...
              className={`w-full text-left p-5 border-b border-slate-50 flex justify-between items-center transition-all ${!filterValue ? 'bg-indigo-50 border-r-4 border-r-indigo-500' : 'hover:bg-slate-50'}`}
            >
              <span className="text-sm font-black text-slate-800 uppercase tracking-wider">Show All Records</span>
              <span className="bg-indigo-600 text-white text-[10px] font-black px-2.5 py-1 rounded-full">{stats.total}</span>
            </button>
            {filteredCounsellors.map(([name, count]) => (
              ...
              >
                <span className="text-sm font-bold text-slate-700 truncate mr-4">{name}</span>
                <span className="bg-slate-100 text-slate-500 text-[10px] font-black px-2.5 py-1 rounded-full group-hover:bg-indigo-100 transition-colors">{count}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-8 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col min-h-0 overflow-hidden">
          <div className="p-6 border-b border-slate-50 flex justify-between items-center shrink-0">
            <div>
              <h3 className="text-xl font-black text-slate-900 truncate max-w-[400px]">
                {!filterValue ? 'Master Directory' : filterValue}
              </h3>
      ...
      {selectedStudent && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden p-8 animate-in zoom-in-95 duration-200">
