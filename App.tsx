...
const App: React.FC = () => {
  ...
  const loadData = useCallback(async (showLoading = true) => {
    ...
    try {
      // 1. Always prioritize Global Cloud Master
      const cloudUrl = await CloudDB.fetchGlobalSheetUrl();
      const localUrl = CloudDB.getSheetUrl();
      const finalUrl = cloudUrl || localUrl;

      if (finalUrl) {
        // Sync state if cloud provided a newer/different link
        if (cloudUrl && cloudUrl !== localUrl) {
          localStorage.setItem('edubase_google_sheet_url', cloudUrl);
          setSheetUrlInput(cloudUrl);
        }
        ...
        if (localCache) setStudents(JSON.parse(localCache));
      }
    } catch (e) {
  ...
  // SYNC ACROSS TABS (Same device)
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'edubase_google_sheet_url' || e.key === DB_KEY) {
        console.log("External tab change detected, syncing...");
  ...
  // SYNC ON WINDOW FOCUS (Different devices/sessions)
  useEffect(() => {
    const handleFocus = () => {
      console.log("App focused, checking for global updates...");
      loadData(false); // Background sync
    };
  ...
  const handleSaveSheetUrl = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!sheetUrlInput.trim().includes('docs.google.com/spreadsheets')) {
      return alert("Invalid Google Sheet URL.");
    }
    setSyncing(true);
    try {
      await CloudDB.saveSheetUrl(sheetUrlInput);
      await loadData(false);
      alert("MASTER LINK SAVED GLOBALLY! All devices and tabs will update instantly.");
      setShowCloudSettingsModal(false);
    } catch (e) {
      alert("Global update failed. Using local storage as fallback.");
    } finally {
      setSyncing(false);
    }
  };
  ...
  const handleSync = async () => {
    ...
    setSyncing(false);
    setIsMenuOpen(false);
    alert("Global database synchronized.");
  };
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="max-w-4xl w-full">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-500 mt-2 text-lg">Cross-Device Student Hub</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <button onClick={() => setRole('user')} className="group bg-white p-10 rounded-[2.5rem] border-2 border-slate-100 text-left hover:border-indigo-500 hover:shadow-2xl transition-all">
              ...
              <p className="text-slate-500">Access the shared database instantly.</p>
            </button>
            <button onClick={() => setShowPasswordPrompt(true)} className="group bg-slate-900 p-10 rounded-[2.5rem] text-left hover:shadow-2xl transition-all">
              ...
              <p className="text-slate-400">Configure global links & analytics.</p>
            </button>
          </div>
        </div>
        ...
        {showPasswordPrompt && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <form onSubmit={handleAdminLogin} className="relative bg-white p-8 rounded-[2rem] shadow-2xl w-full max-w-md animate-in zoom-in-95 duration-200">
              <h3 className="text-xl font-black text-slate-800 mb-2 text-center">Admin Verify</h3>
              ...
              <button type="submit" className="w-full py-4 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 shadow-lg active:scale-95 transition-all">Enter Dashboard</button>
            </form>
          </div>
        )}
      </div>
                ...
                <div className="absolute right-0 top-full pt-2 w-56 animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="bg-white border border-slate-100 rounded-2xl shadow-2xl overflow-hidden p-2">
                    <div className="px-4 py-3 border-b border-slate-50 mb-1">
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Global Identity</p>
                       <p className="text-sm font-bold text-slate-800">{role === 'admin' ? 'Master Admin' : 'Staff User'}</p>
                    </div>
                    {role === 'admin' && (
                      <>
                        ...
                        <button onClick={handleSync} disabled={syncing} className="w-full flex items-center space-x-3 px-4 py-3 text-left text-sm font-bold text-slate-600 hover:bg-slate-50 hover:text-indigo-600 rounded-xl transition-all">
                          ...
                          <span>{syncing ? 'Syncing...' : 'Force Cloud Sync'}</span>
                        </button>
      ...
      <main className="max-w-7xl mx-auto px-4 pt-8 flex-1 w-full overflow-hidden flex flex-col">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-64 text-slate-400 font-bold space-y-4">
             ...
             <p className="animate-pulse tracking-widest text-xs uppercase font-black">Connecting to Master Cloud...</p>
          </div>
          ...
          <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 w-full">
            <div className="text-center mb-12">
              ...
              <div className="relative">
                <input
                  type="text"
                  placeholder="Student Name, SID, or Parent Mobile..."
              ...
              <div className="mt-4 flex items-center justify-center space-x-2">
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Global Master Link Connected</p>
              </div>
            </div>
      ...
      {showCloudSettingsModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <form onSubmit={handleSaveSheetUrl} className="relative w-full max-w-xl bg-white rounded-[2.5rem] shadow-2xl p-10 animate-in zoom-in-95 duration-200">
             ...
             <h3 className="text-2xl font-black text-slate-900 mb-2">Master Cloud Setup</h3>
             <p className="text-slate-500 mb-8 text-sm leading-relaxed">
               Enter your Google Sheet link below. Once saved, **every user on every device** will automatically sync to this link the next time they open the app or refresh.
             </p>
             <div className="space-y-4">
               ...
               <button type="submit" disabled={syncing} className="w-full py-4 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-all shadow-lg flex items-center justify-center space-x-2">
                 ...
                 <span>{syncing ? 'Pushing to Cloud Master...' : 'Update Globally for All Devices'}</span>
               </button>
             </div>
          </form>
        </div>
